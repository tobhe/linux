...
#define mvx_to_scmi_pd(pd) container_of(pd, struct mvx_scmi_perf_domain, genpd)

struct mvx_scmi_perf_domain {
...
static void notify_dsm_event(struct mvx_client_session *csession, char *msg)
{
    if (IS_ERR_OR_NULL(csession))
        return;

    if (IS_ERR_OR_NULL(csession->ctx))
        return;

    dsm_send_event(csession->ctx->mvx_dsm_client, msg);
    return;
}
...
static int mvx_scmi_device_set_freq(struct device *dev, unsigned long freq)
{
    struct generic_pm_domain *genpd = pd_to_genpd(dev->pm_domain);
    struct mvx_scmi_perf_domain *pd = mvx_to_scmi_pd(genpd);
    int ret;
...
static unsigned long mvx_scmi_device_get_freq(struct device *dev)
{
    struct generic_pm_domain *genpd = pd_to_genpd(dev->pm_domain);
    struct mvx_scmi_perf_domain *pd = mvx_to_scmi_pd(genpd);
    unsigned long rate;
...
static int mvx_scmi_device_opp_table_parse(struct device *dev)
{
    struct generic_pm_domain *genpd = pd_to_genpd(dev->pm_domain);
    struct mvx_scmi_perf_domain *pd = mvx_to_scmi_pd(genpd);
    ...
    ph = pd->ph;
     ...
     (KERNEL_VERSION(6, 6, 0) <= LINUX_VERSION_CODE && defined(MVX_ANDROID_KERNEL)))
...
static int mvx_devfreq_target(struct device *dev, unsigned long *freq, u32 flags)
{
    ...
    dev_pm_opp_put(opp);
    pre_freq = mvx_scmi_device_get_freq(ctx->opp_pmdomain);
    if (pre_freq == target_freq)
        return ret;

    ret = mvx_scmi_device_set_freq(ctx->opp_pmdomain, *freq);
...
static int mvx_devfreq_get_cur_freq(struct device *dev, unsigned long *freq)
{
    ...
    *freq = mvx_scmi_device_get_freq(ctx->opp_pmdomain);
                            ...
                            struct devfreq_dev_status *stat)
{
    ...
    update_freq(ctx);
    stat->current_frequency = mvx_scmi_device_get_freq(ctx->opp_pmdomain);
...
static int mvx_devfreq_init(struct mvx_dev_ctx *ctx)
{
    ...
    if (IS_ERR_OR_NULL(ctx->opp_pmdomain)) {
        MVX_LOG_PRINT(&mvx_log_dev, MVX_LOG_WARNING, "Failed to get perf domain, skip devfreq initialization");
        return 0;
    }
    ...
    ret = mvx_scmi_device_opp_table_parse(ctx->opp_pmdomain);
    if (ret) {
             ...
             int irq)
{
    ...
    ctx->mvx_dsm_client = mvx_dsm_init();
    ...
    MVX_LOG_PRINT(&mvx_log_dev, MVX_LOG_WARNING,
              "Linlon v%x identified. cores=%u, core_mask=0x%x, nlsid=%u, id=%u.",
              mvx_hwreg_get_hw_id(&ctx->hwreg),
              mvx_hwreg_get_ncores(&ctx->hwreg),
              mvx_hwreg_get_core_mask(&ctx->hwreg),
...
static int mvx_dev_remove(struct mvx_dev_ctx *ctx)
{
    ...
    free_irq(ctx->irq, ctx);
    mvx_dsm_remove(ctx->mvx_dsm_client);
    mvx_devfreq_remove(ctx);
    mvx_cpufreq_remove(ctx);
...
static int mvx_switch_qchannel_clock_gating(struct mvx_dev_ctx *ctx, bool enable)
{
    ...
    if (ctx->hwreg.hw_ver.svn_revision == MVE_SVN_ENPWOFF) {
        ...
        reg = mvx_hwreg_read_rcsu(&ctx->hwreg, MVX_RCSU_HWREG_SKY1P_CLOCK_QCHANNEL_CONTROL);
        reg = enable ? (reg | MVX_RCSU_HWREG_SKY1P_CLOCK_QCHANNEL_ENABLE) :
                    (reg & ~(MVX_RCSU_HWREG_SKY1P_CLOCK_QCHANNEL_ENABLE));
        mvx_hwreg_write_rcsu(&ctx->hwreg, MVX_RCSU_HWREG_SKY1P_CLOCK_QCHANNEL_CONTROL, reg);
    }
...
static int mvx_pm_runtime_resume(struct device *dev)
{
    ...
    if (ctx->hwreg.hw_ver.svn_revision == MVE_SVN_4K) {
        ...
        arm_smccc_smc(CIX_SIP_RESTORE_MMHUB_ASNI, VPU0_ASNI_ID, 0, 0, 0, 0, 0, 0, &res);
        if (res.a0) {
            MVX_LOG_PRINT(&mvx_log_dev, MVX_LOG_ERROR, "Failed to restore VPU0 ASNI: %d", res.a0);
            return -EPERM;
        }
        arm_smccc_smc(CIX_SIP_RESTORE_MMHUB_ASNI, VPU1_ASNI_ID, 0, 0, 0, 0, 0, 0, &res);
        if (res.a0) {
            MVX_LOG_PRINT(&mvx_log_dev, MVX_LOG_ERROR, "Failed to restore VPU1 ASNI: %d", res.a0);
            return -EPERM;
        }
    }
