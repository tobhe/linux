                 ...
                 long error)
{
    ...
    sprintf(dsm_msg, "session %p send error event,code is %ld", session, error);
    session->client_ops->notify_dsm_event(session->csession, dsm_msg);
}
...
{
    if ((dir == MVX_DIR_INPUT || session->orig_width < 144 || session->orig_height < 144) &&
        *width >= 0 && *height >= 0) {
              ...
              bool is_jpeg)
{
    ...
    session->port[MVX_DIR_OUTPUT].buffer_min_afbc = 1;
    session->port[MVX_DIR_OUTPUT].buffer_min_planar = 1;
    ...
    mutex_init(&session->port[MVX_DIR_OUTPUT].port_changing_mutex);
    mutex_init(&session->port[MVX_DIR_INPUT].port_changing_mutex);
    session->crop_left = 0;
                 ...
                 uint64_t *formats)
{
    ...
    if (!session->is_jpeg) {
        mvx_clear_bit(MVX_FORMAT_JPEG, &fw_formats);
    } else {
        mvx_clear_bit(MVX_FORMAT_H263, &fw_formats);
        mvx_clear_bit(MVX_FORMAT_H264, &fw_formats);
        mvx_clear_bit(MVX_FORMAT_HEVC, &fw_formats);
        mvx_clear_bit(MVX_FORMAT_MPEG2, &fw_formats);
        mvx_clear_bit(MVX_FORMAT_MPEG4, &fw_formats);
        mvx_clear_bit(MVX_FORMAT_VP8, &fw_formats);
        mvx_clear_bit(MVX_FORMAT_VP9, &fw_formats);
        mvx_clear_bit(MVX_FORMAT_AV1, &fw_formats);
    }
    *formats &= fw_formats;
}
               ...
               bool *interlaced)
{
    ...
    /* If decode output format changes between planar and afbc, update min_buffer */
    if (dir == MVX_DIR_OUTPUT && !session->is_encoder &&
        mvx_is_afbc(port->format) != mvx_is_afbc(format)) {
        if (mvx_is_afbc(format) != false)
            port->buffer_min = port->buffer_min_afbc;
        else
            port->buffer_min = port->buffer_min_planar;
    }
             ...
             enum mvx_direction dir)
{
     ...
     */
    if (dir == MVX_DIR_OUTPUT) {
        mutex_lock(&session->port[dir].port_changing_mutex);
        if (session->port[dir].in_port_changing) {
            if (session->port[dir].get_format_called) {
                return_done_buffers(session, dir);
                ret = flush_and_qbufs(session, dir);
        ...
        mutex_unlock(&session->port[dir].port_changing_mutex);
        if (ret != 0 && ret != -EAGAIN) {
            MVX_SESSION_ERR(session, "Failed to queue pending buffers.");
            return ret;
        }
    }
...
static void mvx_handle_alloc_param (struct mvx_session *session, struct mvx_fw_msg *msg)
{
    ...
    mutex_lock(&output->port_changing_mutex);
    ...
    if (output->in_port_changing) {
        ...
        output->get_format_called = false;
    }
    mutex_unlock(&output->port_changing_mutex);
...
static void mvx_handle_seq_param(struct mvx_session *session, struct mvx_fw_msg *msg)
{
    ...
    mutex_lock(&p->port_changing_mutex);
    p->buffer_min_afbc = msg->seq_param.afbc.buffers_min;
    p->buffer_min_planar = session->port[MVX_DIR_INPUT].format == MVX_FORMAT_AV1 ?
                            MVX_DECODE_AV1_PLANNAR_BUFFER_NUM : MVX_DECODE_PLANNAR_BUFFER_NUM;
    p->buffer_min_planar = p->new_width * p->new_height <= 8192 * 8192 ?
                        max(msg->seq_param.planar.buffers_min, p->buffer_min_planar) :
                        msg->seq_param.planar.buffers_min;
    if (mvx_is_afbc(p->format) != false) {
        p->in_port_changing |= msg->seq_param.afbc.buffers_min != p->buffer_min;
        p->buffer_min = p->buffer_min_afbc;
        ...
        p->buffer_min = p->buffer_min_planar;
    }
    ...
    if (p->in_port_changing)
        p->get_format_called = false;
    mutex_unlock(&p->port_changing_mutex);
