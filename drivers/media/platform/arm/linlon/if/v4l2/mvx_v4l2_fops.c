...
int mvx_v4l2_release(struct file *file)
{
    ...
    if (!wait_for_completion_timeout(&vsession->cmp, msecs_to_jiffies(15000))) {
        MVX_SESSION_ERR(&vsession->session, "Wait destroy session complete timeout, ref=%u.",
                mvx_session_ref_read(&vsession->session));
        kref_init(&vsession->session.isession.kref);
        mvx_session_put(&vsession->session);
        wait_for_completion_timeout(&vsession->cmp, msecs_to_jiffies(15000));
    }
