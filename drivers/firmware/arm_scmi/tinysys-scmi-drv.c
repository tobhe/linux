	...
	struct scmi_tinysys_slbc_ctrl_status *rvalue)
{
	...
	if (ret)
		dev_info(&(t_info->sdev->dev), "[scmi][tinysys_slbc_ctrl] ret:%d cmd:%u slbc_resv1:%u slbc_resv2:%u slbc_resv3:%u slbc_resv4:%u slbc_ret:%u slbc_resv1:%u, slbc_resv2:%u, s;bc_resv3:%u, slbc_resv4:%u\n",
...
int scmi_tinysys_event_notify(u32 feature_id, u32 notify_enable)
{
	...
	if (notify_enable) {
		...
		if (ret)
			dev_info(&(t_info->sdev->dev), "scmi register_event_notifier f_id:%d ret:%d\n", f_id, ret);

	} else {
		ret = sdev->handle->notify_ops->devm_event_notifier_unregister(sdev,
			 SCMI_PROTOCOL_TINYSYS, SCMI_EVENT_TINYSYS_NOTIFIER, &f_id, &tinysys_nb);
		if (ret)
			dev_info(&(t_info->sdev->dev), "scmi unregister_event_notifier f_id:%d ret:%d\n", f_id, ret);
	}
...
void scmi_tinysys_register_event_notifier(u32 feature_id, f_handler_t hand)
{
	if (feature_id < SCMI_TINYSYS_CB_MAX)
		...
		dev_info(&(t_info->sdev->dev), "feature_id %d >= SCMI_TINYSYS_CB_MAX\n", feature_id);
}
