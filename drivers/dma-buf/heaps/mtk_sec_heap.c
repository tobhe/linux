...
#if IS_ENABLED(CONFIG_MTK_TRUSTONIC_SVP)
...
#if IS_ENABLED(CONFIG_MTK_TRUSTONIC_SVP)
static struct sg_table *
mtk_sec_heap_region_map_dma_buf_svp(struct dma_buf_attachment *attachment,
				    enum dma_data_direction direction)
{
	struct dma_heap_attachment *a = attachment->priv;
	struct sg_table *table = a->table;
	...
	if (buffer->len <= 0 || buffer->len > sec_heap->region_size ||
		...
		pr_notice("%s err. req size/pa is invalid! heap:%s\n", __func__,
	...
	ret = dma_map_sgtable(attachment->dev, table, direction, attr);
	if (ret) {
		pr_notice("%s err, iommu-dev(%s) dma_map_sgtable failed\n",
		       __func__, dev_name(attachment->dev));
		mutex_unlock(&buffer->map_lock);
		return ERR_PTR(ret);
	}
...
#if IS_ENABLED(CONFIG_MTK_TRUSTONIC_SVP)
	.map_dma_buf = mtk_sec_heap_region_map_dma_buf_svp,
#else
