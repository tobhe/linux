...
static const struct dma_map_ops *smmu_dma_ops = NULL;
			    ...
			    u64 tab_id, unsigned int dom_id)
{
	...
	if (!fwspec || tab_id >= MTK_M4U_TAB_NR_MAX ||
...
{
	if (likely(smmu_dma_ops))
		return;

	if (dev_iommu_priv_get(dev) && dev->dma_ops)
						...
						enum dma_data_direction direction)
{
	...
	if (fwspec) {
		tab_id = mtk_smmu_v3_get_smmu_tab_id(attachment->dev);
		cache_data = get_iova_cache(buffer, tab_id, coherent);
	}

	/* device with iommus attribute AND mapped before */
	if (fwspec && cache_data != NULL && cache_data->mapped[0]) {
		/* mapped before, return saved table */
		ret = copy_sg_table(cache_data->mapped_table[0], table);
	...
	if (dma_map_sgtable(attachment->dev, table, direction, attr)) {
		pr_info("%s map fail tab:%llu, dev:%s\n",
			__func__, tab_id, dev_name(attachment->dev));
		mutex_unlock(&buffer->map_lock);
		...
		if (dmabuf_rbtree_dump_by_domain)
			dmabuf_rbtree_dump_by_domain(tab_id, 0);
...
static struct dma_buf *mtk_slc_heap_allocate(struct dma_heap *heap,
					    ...
					    unsigned long heap_flags)
{
	return system_heap_do_allocate(heap, len, fd_flags,
				       heap_flags | DMA_HEAP_FLAGS_MTK_SLC,
				       false,
