...
struct dump_fd_data {
	...
	struct list_head count_list_v_head;
};
...
static void dmdbuf_clear_size(struct dump_fd_data *fddata)
{
	...
	list_for_each_entry_safe(plist, tmp_plist, &fddata->count_list_head, list_node) {
		...
		kfree(plist->exp_name);
		kfree(plist->name);
		kfree(plist);
	}

	list_for_each_entry_safe(plist, tmp_plist, &fddata->count_list_v_head, list_node) {
		list_del(&plist->list_node);
		kfree(plist->exp_name);
		kfree(plist->name);
		kfree(plist);
...
static bool mtk_dmabuf_is_vhost_dmabuf(const struct dma_buf *dmabuf)
{
	return !strcmp(dmabuf->exp_name, "Vhost DmaBuf");
}
			     ...
			     void *priv)
{
	...
	/*
	 * Vhost DmaBuf comes from android,
	 * When calculating the total size, exclude the portion of Vhost DmaBuf.
	 */
	if (!heap || is_dmabuf_from_heap(dmabuf, heap))
		if (!mtk_dmabuf_is_vhost_dmabuf(dmabuf))
					   ...
					   unsigned long flag)
{
	...
	INIT_LIST_HEAD(&fddata->count_list_v_head);
	kfree(pids);
	return fddata;
}
...
static void dmabuf_count_size(struct dump_fd_data *fddata, const struct dma_buf *dmabuf)
{
	...
	list_for_each_entry_safe(plist, n, &fddata->count_list_head, list_node) {
		if ((plist->name && dmabuf->name && strcmp(dmabuf->name, plist->name) == 0) ||
		      (!plist->name && !dmabuf->name && strcmp(dmabuf->exp_name, plist->exp_name) == 0)) {
	...
	list_for_each_entry_safe(plist, n, &fddata->count_list_v_head, list_node) {
		if ((plist->name && dmabuf->name && strcmp(dmabuf->name, plist->name) == 0) ||
		      (!plist->name && !dmabuf->name && strcmp(dmabuf->exp_name, plist->exp_name) == 0)) {
			plist->count += file_count(dmabuf->file) - 1;
			plist->size += (unsigned long) (dmabuf->size / 1024);
	...
	if (dmabuf->exp_name != NULL) {
		exp_name = kzalloc(strlen(dmabuf->exp_name) + 1, GFP_ATOMIC);
		if (!exp_name) {
			kfree(new_info);
			kfree(name);
			return;
		}
		memcpy(exp_name, dmabuf->exp_name, strlen(dmabuf->exp_name) + 1);
	}
	new_info->name = name;
	new_info->exp_name = exp_name;
	new_info->size = (unsigned long) (dmabuf->size / 1024);
	new_info->count = file_count(dmabuf->file) - 1;

	if (dmabuf->exp_name && !mtk_dmabuf_is_vhost_dmabuf(dmabuf))
		list_add_tail(&new_info->list_node, &fddata->count_list_head);
	else
		list_add_tail(&new_info->list_node, &fddata->count_list_v_head);
}

static void dmabuf_count_list_dump(struct seq_file *s, struct list_head *count_list_h, u64 total_size)
{
	...
	list_sort(NULL, count_list_h, dmabuf_size_cmp);
	/* dump top max user */
	list_for_each_entry_safe(p_count_list, n_count, count_list_h, list_node) {
...
static void dmabuf_count_list_top_dump(struct dump_fd_data *fddata, u64 total_size, int total_cnt)
{
	...
	dmabuf_count_list_dump(s, &fddata->count_list_head, total_size);
}

static void dmabuf_count_list_v_top_dump(struct dump_fd_data *fddata, u64 total_size_v, int total_cnt_v)
{
	struct seq_file *s = fddata->constd.s;

	dmabuf_dump(s, "Vhost DmaBuf comes from android\n");
	dmabuf_dump(s, "\n");
	dmabuf_dump(s, "Vhost DmaBuf total_cnt:%d, total_size:%lluKB, top user:\n",
		    total_cnt_v, total_size_v);
	dmabuf_dump(s, "\t%-35s %-20s %-25s\n",
		    "debug name", "sum of size(KB)", "count of heap");

	dmabuf_count_list_dump(s, &fddata->count_list_v_head, total_size_v);
...
void dmabuf_rbtree_dump_domain(u64 tab_id, u32 dom_id)
{
	...
	const struct dma_buf *dmabuf;
	struct dmabuf_debug_node *dbg_node;
	int total_cnt = 0, total_cnt_v = 0;
	u64 size = 0, total_size = 0, total_size_v = 0;
	struct dump_fd_data *fddata;
	...
	for (tmp_rb = rb_first(rbroot); tmp_rb; tmp_rb = rb_next(tmp_rb)) {
		...
		if (!mtk_dmabuf_is_vhost_dmabuf(dmabuf)) {
			...
			total_size_v += size;
			total_cnt_v += file_count(dmabuf->file) - 1;
		}
	}
	dmabuf_count_list_top_dump(fddata, total_size, total_cnt);
	dmabuf_count_list_v_top_dump(fddata, total_size_v, total_cnt_v);
...
size_t mtk_dmabuf_get_pid_usage(pid_t pid)
{
	int i = 0;

	if (gmap == NULL) {
		pr_info("[%s]err: gmap is NULL\n", __func__);
		return 0;
	}

	while (gmap[i].id != 0 && i < TOTAL_PID_CNT) {
		if (pid == gmap[i].id)
			return gmap[i].PSS;
		i++;
	}
			       ...
			       unsigned long nb_val, void *nb_freed)
{
	...
	if (!gmap) {
		gmap = kcalloc(TOTAL_PID_CNT, sizeof(struct pid_map), DMA_HEAP_DUMP_ALLOC_GFP);
		if (gmap == NULL)
			return 0;
	}
	/* get pid map */
	fddata = dmabuf_rbtree_add_all(NULL, NULL, -1, HEAP_DUMP_PSS_BY_PID);
	if (IS_ERR_OR_NULL(fddata)) {
		pr_info("[%s]err: no memory fddata\n", __func__);
		return 0;
	}
	cnt = dmabuf_rbtree_cal_stats(fddata, &krn_rss, HEAP_DUMP_PSS_BY_PID);
	if (cnt > TOTAL_PID_CNT)
		cnt = TOTAL_PID_CNT;
	map = fddata->pid_map;
	memcpy(gmap, map, cnt * sizeof(struct pid_map));
	dmabuf_dbg_rbtree_clear(fddata);
	...
	if ((dmabuf_total_size + pool_total_size) / PAGE_SIZE > (totalram_pages() / 2))
		pr_info("%s: Memory pressure! dmabuf usage exceeds half of total\n", __func__);
	mtk_dmabuf_dump_all(NULL, HEAP_DUMP_OOM | HEAP_DUMP_STATISTIC | HEAP_DUMP_PSS_BY_PID);
