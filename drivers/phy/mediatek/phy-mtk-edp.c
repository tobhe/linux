...
static void mtk_dptx_phyd_reset_swing_pre(struct mtk_edp_phy *dp_phy)
{
...
{
	struct mtk_edp_phy *dp_phy = phy_get_drvdata(phy);
...
int mtk_edp_phy_configure(struct phy *phy, union phy_configure_opts *opts)
{
	struct mtk_edp_phy *dp_phy = phy_get_drvdata(phy);
	u32 val;
