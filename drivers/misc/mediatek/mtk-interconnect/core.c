...
int mtk_icc_provider_del(struct icc_provider *provider)
{
	mutex_lock(&icc_lock);
	if (provider->users) {
		dev_warn(provider->dev, "interconnect provider still has %d users\n",
			provider->users);
		mutex_unlock(&icc_lock);
	...
	if (!list_empty(&provider->nodes)) {
		dev_warn(provider->dev, "interconnect provider still has nodes\n");
		mutex_unlock(&icc_lock);
